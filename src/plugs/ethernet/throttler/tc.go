...
func addNetemRule(cfg *Config, c commander) error {
	...
	if cfg.Reorder > 0 {
		strs = append(strs, fmt.Sprintf(tcReorder, strconv.FormatFloat(cfg.Reorder, 'f', 2, 64)))
	}

	if cfg.Duplicate > 0 {
		strs = append(strs, fmt.Sprintf(tcDuplicate, strconv.FormatFloat(cfg.Duplicate, 'f', 2, 64)))
	}
